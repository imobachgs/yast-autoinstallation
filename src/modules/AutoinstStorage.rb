...
# storage-ng
    ...
    # @option settings [Hash]        "control_partitioning" Settings to override the control.xml
    # @option settings [Array<Hash>] "partitioning"         List of drives
    ...
    def Import(settings)
      ...
      if settings.is_a?(Hash) || settings.empty?
        ...
        custom_settings = settings.is_a?(Hash) ? settings.fetch("proposal", {}) : {}
        initialize_proposal(custom_settings)
      else
        false
      end
    end
    ...
    def initialize_proposal(settings)
